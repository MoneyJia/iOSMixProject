...
NSString * randomStrGen(NSString *number);
...
void modifyResouceNamePrefix(NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames,NSString *sourceUrl);
NSMutableArray * getAllSource(NSMutableString *projectContent, NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames,NSString *sourceUrl);
...
void modifySourceName(NSString *sourceCodeDir, NSString *oldClassName, NSString *newClassName);
///修改通知名字
void modifyNotifacationNamePrefix(NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames,NSString *nameContent);
void modifyNotifacaitonName(NSString *sourceCodeDir, NSString *oldClassName, NSString *newClassName);


///修改本地plist文件
void modifyPlistNamePrefix(NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames) ;
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        NSString *souceUrl = nil;
        NSString *nameContent = nil;

        NSString *cityUrl = nil;
        ...
        for (NSInteger i = 1; i < arguments.count; i++) {
            ...
            if ([argument isEqualToString:@"-modifyResouceNamePrefix"]) {
                souceUrl = arguments[++i];
                if (![fm fileExistsAtPath:souceUrl isDirectory:&isDirectory] || !isDirectory) {
                    printf("修改资源名的工程文件参数错误。%s", souceUrl.UTF8String);
                    return 1;
                }
                continue;
            }
            ...
            if ([argument isEqualToString:@"-modifyPlistNamePrefix"]) {
                cityUrl = @"ddd";
                if (cityUrl.length == 0) {
                    printf("修改资源名的工程文件参数错误。%s", cityUrl.UTF8String);
                    return 1;
                }
                continue;
            }
            
            if ([argument isEqualToString:@"-modifyNotifacationNamePrefix"]) {
                nameContent = arguments[++i];
                if (nameContent.length == 0) {
                    printf("需要替换的资源名字不对。参数示例：aaa,bbbb，传入参数：%s\n", nameContent.UTF8String);
                    return 1;
                }
                continue;
            }
...
//
        ...
        if (souceUrl) {
            printf("开始修改资源名...\n");
            @autoreleasepool {
                modifyResouceNamePrefix(gSourceCodeDir, ignoreDirNames, souceUrl);
            }
            printf("修改资源名完成\n");
        }
        ...
        if (cityUrl) {
            printf("开始修改plist资源名...\n");
            @autoreleasepool {
                modifyPlistNamePrefix(gSourceCodeDir, ignoreDirNames);
            }
            printf("修改plist资源名完成\n");
        }
        ...
        if (nameContent) {
            printf("开始修改通知信息...\n");
            @autoreleasepool {
                modifyNotifacationNamePrefix(gSourceCodeDir, ignoreDirNames, nameContent);
            }
            printf("修改通知信息完成\n");
        }
...
void modifyClassNamePrefix(NSMutableString *projectContent, NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames, NSString *oldName, NSString *newName) {
    ...
    for (NSString *filePath in files) {
        ...
        /*
         old nn nannan *nannan *nn 这四个
         */
        NSString *oldName2 = @"YB";
        NSString *oldName3 = @"CP";
        NSString *oldName4 = @"Nannan";

        if ([fileName hasPrefix:oldName]) {
            newClassName = [newName stringByAppendingString:[fileName substringFromIndex:oldName.length]];
            NSString *randStr = randomStrGen(@"2");
            newClassName = [newClassName stringByAppendingString:randStr];
        } else if ([fileName hasPrefix:oldName2]) {
            newClassName = [newName stringByAppendingString:[fileName substringFromIndex:oldName2.length]];
        } else if ([fileName hasPrefix:oldName3]) {
            newClassName = [newName stringByAppendingString:[fileName substringFromIndex:oldName3.length]];
        } else if ([fileName hasPrefix:oldName4]) {
            newClassName = [newName stringByAppendingString:[fileName substringFromIndex:oldName4.length]];
        } else {
            
            NSString *newfileName = [fileName substringFromIndex:1];
            
            if ([newfileName hasPrefix:oldName]) {
                newClassName = [newName stringByAppendingString:[newfileName substringFromIndex:oldName.length]];
            } else if ([newfileName hasPrefix:oldName2]) {
                newClassName = [newName stringByAppendingString:[newfileName substringFromIndex:oldName2.length]];
            } else if ([newfileName hasPrefix:oldName3]) {
                newClassName = [newName stringByAppendingString:[newfileName substringFromIndex:oldName3.length]];
            } else if ([newfileName hasPrefix:oldName4]) {
                newClassName = [newName stringByAppendingString:[newfileName substringFromIndex:oldName4.length]];
            } else {
                
                if ([fileName containsString:newName]) {
        ...
        printf("chang file name old name %s, new name %s \n", fileName.UTF8String, newClassName.UTF8String);
...
void modifyPlistNamePrefix(NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames) {
    ...
    NSDictionary*  dic = [NSDictionary dictionaryWithContentsOfFile:@"/Users/admin/Desktop/work/混淆/iOSMixProject-master/ProjectMix/commen.plist"];

    NSDictionary *urlParams = [dic objectForKey:@"other"];
    NSArray *urls = urlParams.allKeys;
    ...
    for (NSString *urlKey in urls) {
        
        ///旧的key
        NSString *oldname = urlKey;
        
        ///新的key
        NSString *newName = md5Str(urlKey);

        
        ///存入新字典
        NSString *content = [urlParams objectForKey:urlKey];
        [newDic setValue:content forKey:newName];
        
        //内容
        @autoreleasepool {
            modifySourceName(gSourceCodeDir, oldname, newName);
        }
    ...
    NSMutableDictionary *allDic = [NSMutableDictionary dictionaryWithDictionary:dic];
    [allDic removeObjectForKey:@"other"];
    [allDic setValue:newDic forKey:@"other"];
    
    BOOL isSus = [allDic writeToFile:@"/Users/admin/Desktop/xcode/commen.plist" atomically:YES];
    
    printf("------");
    
    
//    NSDictionary *urlParams = [dic objectForKey:@"appUrlSecretKey"];
//    NSArray *urls = urlParams.allKeys;
//
//
//    NSDictionary *speciakUrlParams = [dic objectForKey:@"special"];
//
//    NSMutableDictionary *newDic = [NSMutableDictionary dictionary];
//
//    NSMutableDictionary *newDic2 = [NSMutableDictionary dictionary];
//
//    int index = 0;
//
//
...
//
//
//        NSArray *specialArray = speciakUrlParams.allKeys;
//
//        if([specialArray containsObject:urlKey]) {
//
//            NSDictionary *specilDic = [speciakUrlParams objectForKey:urlKey];
//            [newDic2 setValue:specilDic forKey:newName];
//        }
//
//        //内容
//        @autoreleasepool {
...
//
//    NSMutableDictionary *allDic = [NSMutableDictionary dictionaryWithDictionary:dic];
//    [allDic removeObjectForKey:@"appUrlSecretKey"];
//    [allDic setValue:newDic forKey:@"appUrlSecretKey"];
//
//    [allDic removeObjectForKey:@"special"];
//    [allDic setValue:newDic2 forKey:@"special"];
//
//    BOOL isSus = [allDic writeToFile:@"/Users/admin/Desktop/xcode/commen.plist" atomically:YES];
//
//    printf("------");
...
void modifyResouceNamePrefix(NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames,NSString *sourceUrl) {
    ...
    NSFileManager *fm = [NSFileManager defaultManager];
    NSArray<NSString *> *files = [fm contentsOfDirectoryAtPath:sourceUrl error:nil];
    
    BOOL isDirectory;
    for (NSString *filePath in files) {
        NSString *path = [sourceUrl stringByAppendingPathComponent:filePath];
        if ([fm fileExistsAtPath:path isDirectory:&isDirectory] && isDirectory) {
            modifyResouceNamePrefix(sourceCodeDir, ignoreDirNames, path);
            continue;
        }
        ///概率修改
        NSInteger k = arc4random()%100;
        ...
        if ([fileExtension isEqualToString:@"json"]) {
            ...
            ///替换
            //1,随机字符串
            NSString *randStr = [NSString stringWithFormat:@"ad_%@",randomStrGen(@"10")];
            
            //2.替换
            NSString *oldFilePath = [[sourceUrl stringByAppendingPathComponent:fileName] stringByAppendingPathExtension:fileExtension];
            NSString *newFilePath = [[sourceUrl stringByAppendingPathComponent:randStr] stringByAppendingPathExtension:@"json"];
            ...
            //内容
            @autoreleasepool {
                modifySourceName(gSourceCodeDir, fileName, randStr);
            }
            ...
            //2.替换
            NSString *oldFilePath = [[sourceUrl stringByAppendingPathComponent:fileName] stringByAppendingPathExtension:fileExtension];
            NSString *newFilePath = [[sourceUrl stringByAppendingPathComponent:randStr] stringByAppendingPathExtension:@"svga"];
...
NSMutableArray * getAllSource(NSMutableString *projectContent, NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames,NSString *sourceUrl) {
    ...
    for (NSString *filePath in files) {
        NSString *path = [sourceUrl stringByAppendingPathComponent:filePath];
        if ([fm fileExistsAtPath:path isDirectory:&isDirectory] && isDirectory) {
            NSMutableArray *newNames = getAllSource(projectContent, sourceCodeDir, ignoreDirNames, path);
            [names addObjectsFromArray:newNames];
            continue;
        }
        ///概率修改
        NSInteger k = arc4random()%100;
        ...
        if ([fileExtension isEqualToString:@"json"] || [fileExtension isEqualToString:@"svga"]) {
...
void modifySourceName(NSString *sourceCodeDir, NSString *oldClassName, NSString *newClassName) {
    ...
    for (NSString *filePath in files) {
        ...
        ///如果路径下的是文件夹，继续往下走,知道找到一个文件
        if ([fm fileExistsAtPath:path isDirectory:&isDirectory] && isDirectory) {
            modifySourceName(path,oldClassName,newClassName);
            continue;
        }
        NSString *fileName = filePath.lastPathComponent;

        if ([fileName hasSuffix:@".h"] || [fileName hasSuffix:@".m"] || [fileName hasSuffix:@".mm"] || [fileName hasSuffix:@".swift"]) {
            NSError *error = nil;
            NSMutableString *fileContent = [NSMutableString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:&error];
            if (error) {
                printf("打开文件 %s 失败：%s\n", path.UTF8String, error.localizedDescription.UTF8String);
            ...
            if([fileContent containsString:[NSString stringWithFormat:@"\"%@\"",oldClassName]]){
                NSRange range = NSMakeRange(0, fileContent.length);
                [fileContent replaceOccurrencesOfString:[NSString stringWithFormat:@"\"%@\"",oldClassName] withString:[NSString stringWithFormat:@"\"%@\"",newClassName] options:NSCaseInsensitiveSearch range:range];
...
void modifyNotifacaitonName(NSString *sourceCodeDir, NSString *oldClassName, NSString *newClassName) {
    ...
    for (NSString *filePath in files) {
        ...
        ///如果路径下的是文件夹，继续往下走,知道找到一个文件
        if ([fm fileExistsAtPath:path isDirectory:&isDirectory] && isDirectory) {
            modifyNotifacaitonName(path,oldClassName,newClassName);
            continue;
        }
        NSString *fileName = filePath.lastPathComponent;
        ...
        if ([fileName hasSuffix:@".h"] || [fileName hasSuffix:@".m"] || [fileName hasSuffix:@".mm"] || [fileName hasSuffix:@".swift"]) {
            ...
            ///&& ([fileContent containsString:[NSString stringWithFormat:@"%@ object:",oldClassName]] || [fileContent containsString:[NSString stringWithFormat:@"%@)",oldClassName]] || [fileContent containsString:[NSString stringWithFormat:@"%@ ",oldClassName]] || [fileContent containsString:[NSString stringWithFormat:@"%@ selector",oldClassName]])
            if ([fileContent containsString:oldClassName]) {
                NSRange range = NSMakeRange(0, fileContent.length);
                [fileContent replaceOccurrencesOfString:oldClassName withString:newClassName options:NSCaseInsensitiveSearch range:range];
                [fileContent writeToFile:path atomically:YES encoding:NSUTF8StringEncoding error:nil];
...
void modifyNotifacationNamePrefix(NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames,NSString *nameContent) {
    ...
    //得到名字
    NSArray *names = [nameContent componentsSeparatedByString:@","];
    ...
    for (NSString *name in names) {
        ...
        NSString *str1 = @"kNSNotificatio";
        NSString *str2 = @"Key_";
        NSString *str3 = @"key_";

        if ([name containsString:str1]) {
            newName = [@"CTNotificatio" stringByAppendingString:[name substringFromIndex:str1.length]];
        } else if ([name containsString:str2]) {
            newName = [@"CTKey_" stringByAppendingString:[name substringFromIndex:str2.length]];
        } else if ([name containsString:str3]) {
            newName = [@"CTKey_" stringByAppendingString:[name substringFromIndex:str3.length]];
        } else {
            newName = name;
        }
...
//        NSLog(@"oldName = %@, newName = %@",oldName,newName);
        ...
        //内容
        @autoreleasepool {
            modifyNotifacaitonName(gSourceCodeDir, oldName, newName);
        }
    }
...
//            }
        ...
        if ([fileExtension isEqualToString:@"h"]) {
            ...
            } else if ([swiftFileName isEqualToString:@"swift"]) {
                NSString *swiftFilePath = [[sourceCodeDir stringByAppendingPathComponent:fileName] stringByAppendingPathExtension:@"swift"];
                NSError *error = nil;
                NSMutableString *fileContent = [NSMutableString stringWithContentsOfFile:swiftFilePath encoding:NSUTF8StringEncoding error:&error];
                if([fileContent containsString:oldName]){
                    [fileContent replaceOccurrencesOfString:oldName withString:newName options:NSCaseInsensitiveSearch range:NSMakeRange(0, fileContent.length)];
                    [fileContent writeToFile:swiftFilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
                }
            }
            else{
                NSString *hFilePath = [[sourceCodeDir stringByAppendingPathComponent:fileName] stringByAppendingPathExtension:@"h"];
